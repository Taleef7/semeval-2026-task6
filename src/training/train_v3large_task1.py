#!/usr/bin/env python3
"""
Train DeBERTa-v3-large 10-seed ensemble for Task 1

Based on successful probe (F1=0.6942 vs xlarge's 0.6632)
"""
...
def train_single_seed(fold, seed):
    ...
    all_labels = [TASK1_LABELS.index(train_data[i]['clarity_label']) for i in range(len(train_data))]
    ...
    model = Task1Classifier(model_name=model_name, num_labels=3, label_smoothing=0.03)
    ...
    for epoch in range(num_epochs):
        ...
        val_f1 = f1_score(val_labels, val_preds, labels=[0, 1, 2], average='macro')
        ...
        if val_f1 > best_f1:
            ...
            save_dir = project_root / f"runs/task1_v3large_fold{fold}_seed{seed}_{timestamp}"
